...
export class ChecklistDatabase {
  ...
  constructor(treeData: {}) {
    // TREE_DATA is the data that create checklist structure
...
export class TreeChecklistComponent implements OnInit, OnChanges{
  ...
  ngOnInit(): void {
    this._database = new ChecklistDatabase(this.treeData);
    this._database.dataChange.subscribe(data => {
      this.dataSource.data = data;
    });

    this.checklistSelection.changed.subscribe(data => {
      this.selectedData.emit(this.checklistSelection.selected);
    });
    this.channelImg = ChannelImage;
    if (this.formVal && this.formVal.value)
    {
      this.reverseAply();
    }
  }
  ...
  reverseAply(): void
  {
    ...
    if (this.formVal.value.length > 0)
    {
      // item: "Twitter", level: 0, expandable: true
      const selected: any[] = this.formVal.value;
      for (const eachselected of selected)
      {
        for (const eachNode of this.treeControl.dataNodes) {
          if (eachselected.item === eachNode.item)
          {
            if (!eachselected.expandable)
            {
              this.todoItemSelectionToggle(eachNode);
              if (eachNode.level > 0)
              {
                const parent = this.getParentNode(eachNode);
