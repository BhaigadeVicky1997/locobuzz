...
import { AudioUrl, DocumentUrl, VideoUrl } from 'app/core/models/dbmodel/AllBrandsTicketsDTO';
...
export class UseroneviewService {
  ...
  constructor(private _locobuzzmentionService: LocobuzzmentionService) {}
      ...
      for (const mention of mentionList) {
        ...
        if (mention.channelGroup === ChannelGroup.Facebook) {
          ...
          if (mention.channelType === ChannelType.FBMessages) {
            ...
            userOneDTO.channelIcon = 'assets/images/channelicons/facebook.png';
          ...
          userOneDTO.ticketTiming = this._locobuzzmentionService.calculateTicketTimes(
            mention
          );
          ...
          if (mention.mediaContents && mention.mediaContents.length > 0) {
            if (mention.mediaType === MediaEnum.IMAGE) {
              if (mention.mediaContents.length > 0) {
                for (const MediaContentItem of mention.mediaContents) {
                  if (
          ...
          if (mention.channelType !== ChannelType.FBMessages) {
            userOneDTO.LikeCount = mention.mentionMetadata.likeCount  ? mention.mentionMetadata.likeCount : 0;
            userOneDTO.CommentCount = mention.mentionMetadata.commentCount ? mention.mentionMetadata.commentCount : 0; ;
          ...
          if (mention.channelType === ChannelType.DirectMessages) {
            ...
            userOneDTO.channelIcon = 'assets/images/channelsv/Brand_Tweet.svg';
          ...
          if (mention.channelType === ChannelType.InstagramComments) {
            userOneDTO.channelIcon = 'assets/images/channelicons/Instagram_Comment.png';
      } else {
            userOneDTO.channelIcon = 'assets/images/channelicons/instagram.png';
      }
          userOneDTO.authorName = mention.author.name;
          userOneDTO.ticketId = String(mention.ticketID);
          userOneDTO.ticketTiming = this._locobuzzmentionService.calculateTicketTimes(
          ...
          if (mention.mediaContents && mention.mediaContents.length > 0) {
            for (const MediaContentItem of mention.mediaContents) {
                                    if (MediaContentItem.mediaUrl && MediaContentItem.mediaUrl.includes('.mp4'))
                                    {
                                    ...
                                    else if (MediaContentItem.thumbUrl
                                    && (MediaContentItem.thumbUrl.includes('.png')
                                    || MediaContentItem.thumbUrl.includes('.jpg')
                                    || MediaContentItem.thumbUrl.includes('.jpeg')
                                    || MediaContentItem.thumbUrl.includes('.gif')))
        ...
        else if (mention.channelGroup === ChannelGroup.Email)
        {
          ...
          userOneDTO.subject = mention.subject;
                  ...
                  if (mention.mediaType === MediaEnum.VIDEO)
                  {
                                        ...
                                        if (mention.mediaContents && mention.mediaContents.length > 0) {
                                          for (const MediaContentItem of mention.mediaContents) {
                                                        ...
                                                        userOneDTO.iframeUrl.push(`https://www.youtube.com/embed/${VideoID}?rel=0`);
                ...
                else if (mention.channelGroup === ChannelGroup.LinkedIn)
                {
                  ...
                  userOneDTO.channelIcon = `assets/images/channelicons/linkedin.png`;
                  ...
                  if (mention.parentPostSocialID) {
                    try {
                      userOneDTO.authorLink = `https://www.linkedin.com/feed/update/${mention.parentPostSocialID.split('-')[2]}`;
                    }catch (exception)
                    {
                      const LinkedInDetails = mention.parentPostSocialID.split('-');
                      const LinkedInParentPostSocialID = LinkedInDetails.length === 3 ?
                       LinkedInDetails[2] : LinkedInDetails.length === 2 ? LinkedInDetails[1] : LinkedInDetails.length === 1
                       ? LinkedInDetails[0] : '';
                ...
                try {
                  userOneDTO.authorLink = `https://www.linkedin.com/feed/update/${mention.socialID.split('-')[2]}`;
                }catch (exception)
                {
                  const LinkedInDetails = mention.socialID.split('-');
                  const LinkedInSocialID = LinkedInDetails.length === 3 ?
                   LinkedInDetails[2] : LinkedInDetails.length === 2 ? LinkedInDetails[1] : LinkedInDetails.length === 1
                   ? LinkedInDetails[0] : '';
                ...
                else if (mention.channelGroup === ChannelGroup.GoogleMyReview)
                {
                  ...
                  if (mention.channelType === ChannelType.GMBQuestion) {
                    userOneDTO.channelIcon = 'assets/images/channelicons/GMBQuestion.png';
                  } else if (
                    ...
                    userOneDTO.channelIcon = 'assets/images/channelicons/GoogleMyReview.png';
                  }
                ...
                else if (mention.channelGroup === ChannelGroup.WebsiteChatBot)
                {
                  ...
                  userOneDTO.channelIcon = 'assets/images/channelicons/WebsiteChatBot.svg';
...

                  {
                              ...
                              const error = '';
                              let data = null;
                              if (mention.messageType !== 'text' && mention.mediaContents && mention.mediaContents.length > 0) {
                                for (const media of mention.mediaContents) {
