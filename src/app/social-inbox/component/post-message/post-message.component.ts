...
export class PostMessageComponent implements OnInit {
  ...
  private mapwithRespectiveObject(mention: BaseMention): void {
    ...
    if (this.currentBrand === null && this.currentBrand === undefined) {
      ...
      this.ticketHistoryData.IsCategoryLevelBulkReply = this.currentBrand.isCategoryBulkReply;
      this.ticketHistoryData.isBrandLevelBulkReply = this.currentBrand.isBrandBulkReply;
    ...
    this.ticketHistoryData.ticketCustomReply = this.getTicketCustomReply(
      mention
    );
    ...
    if (
      ...
      ((mention.channelType === 10 && mention.isBrandPost) ||
        (mention.channelType === 17 && mention.isBrandPost))
    ) {
      this.ticketHistoryData.isDeleteFromChannel = true;
    }
    this.ticketHistoryData.isSubscriptionActive = mention.isSubscribed ? 'active' : '';
    this.ticketHistoryData.isVisible = (mention.ticketInfo.status === TicketStatus.Close || mention.isBrandPost) ? false : true;

    this.ticketHistoryData.actionLikeEnabled = this.currentUser.data.user.actionButton.likeEnabled;
    this.ticketHistoryData.actionRetweetEnabled = this.currentUser.data.user.actionButton.retweetEnabled;
  }
  ...
  private mapPostByChannel(mention: BaseMention): TicketClient {
    ...
    const assignToname = this.filterService.getNameByID(mention.ticketInfo.assignedTo, this.filterService.fetchedAssignTo);
    this.post = {
      ...
      Userinfo: {
        ...
        image: mention.author.picUrl ? mention.author.picUrl
                                     : 'assets/images/agentimages/sample-image.jpg',
